...
i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    ...
    if (xSemaphoreTake(lcd_mutex, portMAX_DELAY) == pdTRUE) {
        ...
        i2c_master_write(cmd, ini, sizeof(ini), true);
...
uint8_t i, j;
    ...
    if (xSemaphoreTake(lcd_mutex, portMAX_DELAY) == pdTRUE) {
        ...
        for (i = 0; i < OLED_MAX_STR; i++) {
            ...
            i2c_master_write_byte(cmd, 0xB0 | i, true);
            i2c_master_write_byte(cmd, OLED_CONTROL_BYTE_DATA_STREAM, true);
            for (j = 0; j < 128; j++) i2c_master_write_byte(cmd, 0xFF >> (j % OLED_MAX_STR), true);
...
uint8_t dir;
    ...
    if (xSemaphoreTake(lcd_mutex, portMAX_DELAY) == pdTRUE) {
        ...
        i2c_master_write(cmd, sha, sizeof(sha), true);
...
i2c_cmd_handle_t cmd;
    ...
    if (xSemaphoreTake(lcd_mutex, portMAX_DELAY) == pdTRUE) {
        ...
        i2c_master_start(cmd);
        i2c_master_write_byte(cmd, (OLED_I2C_ADDRESS << 1) | I2C_MASTER_WRITE, true);
        i2c_master_write(cmd, first, sizeof(first), true);
        i2c_master_stop(cmd);
        i2c_master_cmd_begin(SSD1306_PORT, cmd, 10/portTICK_PERIOD_MS);
        i2c_cmd_link_delete(cmd);
