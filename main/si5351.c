...
//extern int get_wspr_channel_symbols(char* rawmessage, uint8_t* symbols);


static uint64_t si5351_pll_calc(enum si5351_pll, uint64_t, struct Si5351RegSet *, int32_t, uint8_t);
static uint64_t si5351_multisynth_calc(uint64_t, uint64_t, struct Si5351RegSet *);
static uint64_t si5351_multisynth67_calc(uint64_t, uint64_t, struct Si5351RegSet *);
...
esp_err_t ret;
    ...
    i2c_config_t cnf = {
        ...
        .sda_pullup_en = GPIO_PULLUP_ENABLE,//0;
        .scl_pullup_en = GPIO_PULLUP_ENABLE,//0;
        .master.clk_speed = SIPORT_CLOCK_HZ
    };

    ret = i2c_param_config(SI5351_PORT, &cnf);
    if (ret == ESP_OK) ret = i2c_driver_install(SI5351_PORT, I2C_MODE_MASTER, 0, 0, 0);
...
int si5351_write_xfer(uint8_t reg, uint8_t *data, int count)
{
    ...
    ret = i2c_master_cmd_begin(SI5351_PORT, cmd, 1000 / portTICK_RATE_MS);
...
int si5351_read_xfer(uint8_t reg, uint8_t *data, int count)
{
    ...
    ret = i2c_master_cmd_begin(SI5351_PORT, cmd, 1000 / portTICK_PERIOD_MS);
    i2c_cmd_link_delete(cmd);
    if (ret != ESP_OK) return (ret);

    cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (SI5351_BUS_BASE_ADDR << 1) | I2C_MASTER_READ, true);
...
//
void si5351_start(enum si5351_clock clk, int64_t freq, enum si5351_drive drive)
{
    (void) si5351_set_freq(freq, clk);
    si5351_output_enable(clk, 0);
    si5351_drive_strength(clk, drive);//SI5351_DRIVE_8MA);
}
//
...
bool si5351_init(uint8_t xtal_load_c, uint32_t xo_freq, int32_t corr)
{
    ...
    if (reg_val == 0) {
        ...
        si5351_write(SI5351_CRYSTAL_LOAD, (xtal_load_c & SI5351_CRYSTAL_LOAD_MASK) | 0x12);//0b00010010);
...
void si5351_reset(void)
{
    ...
    uint8_t dat[8] = {0x80};
    si5351_write_xfer(16, dat, sizeof(dat));
    ...
    memset(dat, 0, 3);
    si5351_write_xfer(SI5351_VXCO_PARAMETERS_LOW, dat, 3);
